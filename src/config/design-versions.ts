...
// ═══════════════════════════════════════════════
//  VERSION 9 — "Atrium" (Executive editorial, warm parchment + aged brass)
// ═══════════════════════════════════════════════
const v9: DesignVersion = {
  id: "v9",
  name: "Atrium",
  description:
    "Executive editorial aesthetic built to command trust from C-suite and UHNW clients. Warm parchment background (#F7F1E8) layered with vellum (#EDE4D3), deep warm-black ink (#1C1A17), and aged-brass accent (#8B6F3E) — the palette of a Savile Row tailor's boardroom. Playfair Display serif headlines with italic brass counterpoints, Inter body with generous line-height. Editorial photography (real executives, boardrooms, skylines) replaces animated mockups. No hard edges, no signal colors, no brutalist devices — only quiet confidence, generous whitespace, and the gravity of a private-bank annual report.",
  createdAt: "2026-04-17",
  status: "active",
  colors: {
    primary: "#8B6F3E",
    primaryLight: "rgba(139,111,62,0.08)",
    primaryDark: "#6F5830",
    primaryGlow: "rgba(139,111,62,0.2)",
    dark: "#1C1A17",
    darkMuted: "#2A2721",
    darkBorder: "rgba(247,241,232,0.1)",
    background: "#F7F1E8",
    surfaceLight: "#EDE4D3",
    surfaceMuted: "#E6DCC7",
    cardBorder: "rgba(28,26,23,0.08)",
    textPrimary: "#1C1A17",
    textSecondary: "rgba(28,26,23,0.7)",
    ...
    error: "#8B3A1F",
    starFilled: "#8B6F3E",
    starEmpty: "rgba(28,26,23,0.15)",
  },
  typography: {
    fontFamily: "Playfair Display (display serif) + Inter (body) + Playfair italic (accents)",
    fontWeights: [300, 400, 500, 600, 700],
    headingDisplay: "v9-serif text-[44px] sm:text-[56px] lg:text-[72px] leading-[1.02] tracking-[-0.02em]",
    headingSection: "v9-serif text-[40px] md:text-[56px] leading-[1.08] tracking-[-0.02em]",
    headingCard: "v9-serif text-[24px] md:text-[28px] leading-tight tracking-[-0.01em]",
    body: "text-[17px] leading-[1.7]",
    small: "text-[14px] leading-[1.65]",
    buttonText: "text-[12px] uppercase tracking-[0.24em] font-semibold",
    badgeText: "v9-eyebrow — text-[11px] uppercase tracking-[0.28em] font-semibold text-[#8B6F3E]",
  },
  spacing: { sectionPadding: "py-24 lg:py-32", containerMaxWidth: "max-w-[1440px]", containerPadding: "px-6 md:px-12 lg:px-16", cardPadding: "p-8 lg:p-10", gridGap: "gap-8 lg:gap-16" },
  radius: { button: "rounded-none", card: "rounded-none", input: "rounded-none (underline-only)", badge: "rounded-none", avatar: "rounded-full", image: "rounded-none" },
  shadows: { card: "v9-paper-edge (soft warm drop)", cardHover: "v9-paper-edge amplified", cardPopular: "v9-paper-edge", button: "none — type contrast carries weight", navbar: "none — hairline border only", floating: "v9-paper-edge", chatButton: "shadow-lg shadow-[#1C1A17]/30" },
  animations: { floatingY: [0, 0, 0], floatingDuration: 0, hoverLift: 0, fadeInDuration: 0.9, staggerDelay: 0.15, dropdownDuration: 0.15 },
  components: {
    navbar: { height: "h-20", background: "bg-[#F7F1E8]/90 backdrop-blur-xl border-b border-[#1C1A17]/[0.08]", border: "hairline", logoHeight: "v9-serif text-[24px]", ctaStyle: "bg-[#1C1A17] text-[#F7F1E8] px-6 py-3 text-[11px] uppercase tracking-[0.22em] font-semibold hover:bg-[#8B6F3E]" },
    hero: { background: "#F7F1E8 with warm paper-fibre texture", badgeColor: "v9-eyebrow brass + hairline bar", imageContainer: "aspect-[4/5] photographic plate with v9-paper-edge + floating caption card", imageAnimation: "opacity + gentle scale fade" },
    cards: { style: "generous editorial card on #EDE4D3, Roman numeral accent, brass hairline rule", hoverEffect: "v9-paper-edge appears on hover, 1.04 image zoom", iconContainer: "rounded-full brass border + 10% brass tint" },
    footer: { layout: "Colophon line + 4-column link register + social row", background: "bg-[#1C1A17] text-[#F7F1E8]", socialIconSize: "h-9 w-9 rounded-full brass-hover border" },
    chatbot: { buttonSize: "h-14 w-14 rounded-full", windowSize: "h-[100dvh] w-full sm:h-[520px] sm:w-[380px]", headerColor: "bg-[#1C1A17]", botBubble: "rounded-tl-sm bg-[#EDE4D3] text-[#1C1A17]", userBubble: "rounded-tr-sm bg-[#1C1A17] text-[#F7F1E8]" },
    buttons: { primary: "bg-[#1C1A17] text-[#F7F1E8] px-8 py-4 text-[12px] uppercase tracking-[0.24em] font-semibold hover:bg-[#8B6F3E] duration-500", secondary: "border-b border-[#1C1A17] pb-0.5 text-[12px] uppercase tracking-[0.24em] font-semibold hover:text-[#8B6F3E] hover:border-[#8B6F3E]", ghost: "text-[11px] uppercase tracking-[0.22em] font-semibold text-[#1C1A17]/70 hover:text-[#1C1A17]" },
    forms: { inputStyle: "border-0 border-b border-[#1C1A17]/25 bg-transparent px-0 py-4 text-[15px] text-[#1C1A17] placeholder:text-[#1C1A17]/40 (underline-only)", inputFocus: "focus:border-[#8B6F3E] focus:outline-none focus:ring-0" },
    pricing: { popularHighlight: "v9-paper-edge + brass rule accent", priceSize: "v9-serif text-[64px]" },
  },
  sectionPattern: [
    "#F7F1E8 parchment (hero, cards, invitation)",
    "#EDE4D3 vellum (products, specification chapters)",
    "#1C1A17 ink (testimonials, CTA, footer) — photographic overlays with warm gradient",
  ],
  notes:
    "V9 Atrium is the executive-grade redesign that replaces the earlier brutalist 'Obsidian' draft. Every surface is calibrated to the gaze of a UHNW or C-suite buyer: warm parchment palette, aged-brass hairlines, Playfair Display serif, and real editorial photography sourced from Unsplash (executives in boardrooms, signed documents, professional discussions). Language is reframed ('Request Private Briefing', 'By Private Appointment', 'The Client Register', 'A partial list of those whose standards we serve'). Images are photographic not illustrative. Photos carry warm overlay + brass-tint multiply blend for magazine-plate consistency. Taste stack: high-end-visual-design (primary) + design-taste-frontend (baseline) + minimalist-ui discipline for whitespace, with industrial-brutalist-ui removed.",
};
